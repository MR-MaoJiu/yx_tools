...
extension ExtensionString on String {
  ...
  int? toInt({bool nullOnError = false}) {
    ...
    throw const FormatException('Can only acception double value');
  }
  ...
  bool toBool({bool falseOnError = false}) {
    ...
    throw const FormatException('Can only acception boolean value');
  }

  /// Transform String millisecondsSinceEpoch (DateTime) to DateTime
  DateTime? toDateTime() {
    int? miliseconds = toInt(nullOnError: true);
    if (ObjectUtils.isNullOrBlank(miliseconds)) return null;
    return DateTime.fromMillisecondsSinceEpoch(miliseconds ?? 0);
  }
  ...
  Color get hexToColor => ColorUtils.hexToColor(this);
}
